...
class WinText:
    ...
    def create_text(
        self, player_name, computer_name, player_score, computer_score, number_rounds
    ):
        ...
        self.number_rounds_heading = ft.arial.render(
            "Осталось раундов", True, (colors.yellow)
        )
        self.player_name = ft.roboto.render(player.name, True, (colors.white))
        self.computer_name = ft.roboto.render(computer.name, True, (colors.white))
        self.score_text_heading = ft.roboto.render("Очки", True, (colors.white))

    def blits(self):

        sc.win.blit(self.player_name, (30, 50))
        sc.win.blit(self.score_text_heading, ((sc.width - 80) / 2, sc.height - 100))
